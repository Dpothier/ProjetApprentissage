...
model = CausalTCN(vocabulary.vectors, p_first_layer=dropout_first_layer, p_other_layers=dropout_values)
if use_gpu:
    model = model.cuda()

trainer = TrainerSoftTarget(2, 0.8, use_gpu)

f = open('./results/causal_{}_{}_extreme.txt'.format(weight_decay_values, dropout_values), 'w')
sys.stdout = f

history_file = './history/causal_{}_{}_extreme.pdf'.format(weight_decay_values, dropout_values)
...
torch.save(best_model.state_dict(), './model_dump/causal_{}_{}_best'.format(weight_decay_values, dropout_values))
torch.save(model.state_dict(), './model_dump/causal_{}_{}_last'.format(weight_decay_values, dropout_values))