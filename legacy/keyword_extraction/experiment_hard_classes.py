...
f = open('./results/hard_target_final.txt', 'w')
sys.stdout = f

test_iter = data.Iterator(test, batch_size=batch_size, device=-1 if use_gpu is False else None, repeat=False)
trainer = Trainer()
trainer.validate(absolute_best_model, test_iter, test_extra, use_gpu=use_gpu, class_weight=tags_weight, ann_folder='./data/test_preds_hard')
