...
class Trainer:
    ...
    def validate(self, model, val_loader, val_extra_data, use_gpu=False, class_weight=None, ann_folder=None):
        true = []
        pred = []
        probabilities = []
        ...
        for batch in val_loader:
            ...
            ids = batch.id
            texts = batch.texts
            process_targets, material_targets, task_targets = self._prepare_targets(batch)
            ...
            process_loss, material_loss, task_loss = self._calculate_loss(out_process, process_targets,
                                                                          out_material, material_targets,
                                                                          out_task, task_targets,
                                                                          class_weight=class_weight, use_gpu=use_gpu)
            ...
            self._compute_confidence(process_targets, out_process, probabilities)

            val_loss.extend([process_loss.data[0], material_loss.data[0], task_loss.data[0]])

            self._compute_accuracy(process_targets, process_predictions,
                                   material_targets, material_predictions,
    ...
    def train(self, model, dataset, training_schedule, batch_size, history_file, weight_decay=0, use_gpu=False,
              class_weight=None, patience=10):
        ...
        for schedule in training_schedule:
            ...
            for i in range(schedule[0]):
                ...
                if i == schedule[0] - 1:
                    train_acc, train_loss, train_confidence = self.validate(model, train_iter, train[1], use_gpu,
                                                                       class_weight,
                                                                       ann_folder="./data/train_preds")
                    val_acc, val_loss, val_confidence = self.validate(model, val_iter, val[1], use_gpu, class_weight,
                ...
                print('Epoch {} - Train acc: {:.2f} - Val acc: {:.2f} - Train loss: {:.4f} - Val loss: {:.4f} - Train conf:{} - Val conf: {}'
                      .format(cummulative_epoch, train_acc, val_acc, train_loss, val_loss, train_confidence,
                            val_confidence))
                cummulative_epoch += 1
    ...
    def _compute_confidence(self, process_targets, out_process, probabilities):
        for i in range(out_process.shape[0]):
    ...
    def _compute_accuracy(self, process_targets, process_predictions, material_targets, material_predictions, task_targets, task_predictions, true, pred):
        true.extend(process_targets.data.contiguous().view(
            process_targets.shape[0] * process_targets.shape[1]).cpu().numpy().tolist())
        pred.extend(process_predictions.data.contiguous().view(
            process_predictions.shape[0] * process_predictions.shape[1]).cpu().numpy().tolist())

        true.extend(material_targets.data.contiguous().view(
            material_targets.shape[0] * material_targets.shape[1]).cpu().numpy().tolist())
        pred.extend(material_predictions.data.contiguous().view(
            material_predictions.shape[0] * material_predictions.shape[1]).cpu().numpy().tolist())

        true.extend(task_targets.data.contiguous().view(
            task_targets.shape[0] * task_targets.shape[1]).cpu().numpy().tolist())
        pred.extend(task_predictions.data.contiguous().view(
            task_predictions.shape[0] * task_predictions.shape[1]).cpu().numpy().tolist())