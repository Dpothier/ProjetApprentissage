...
from networks.mnist_mlp_baseline.module import BaselineMnistMLP
...
@click.option('-g', '--gpu', default="gpu2")
    ...
    for learning_rate in learning_rates:
        for weight_decay in weight_decays:
            for dropout_rate in dropout_rates:
                ...
                module = BaselineMnistMLP(dropout_rate=dropout_rate, state_size=32, depth=4)
                ...
                model.fit_generator(mnist_trainset, mnist_devset, epochs=epochs, callbacks=[gradient_clipping, lr_scheduler, best_model_restore])

                test_loss, test_metrics, test_preds = model.evaluate_generator(mnist_trainset, return_pred=True)
                train_loss, train_metrics, train_preds = model.evaluate_generator(mnist_devset, return_pred=True)
