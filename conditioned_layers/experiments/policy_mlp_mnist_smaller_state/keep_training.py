...
@click.command()
@click.option('-m', '--model_file', default="/results/0.0001_0/model.pth")
@click.option('-g', '--gpu', default="gpu0")
def main(model_file, gpu):
    """
    ...
    for learning_rate in learning_rates:
        for weight_decay in weight_decays:
            for dropout_rate in dropout_rates:
                output_folder = output_folder_base +"{}_{}_again/".format(learning_rate, weight_decay)
                results = Results(output_folder)

                save_hyperparameters(results, learning_rate, epochs, batch_size, weight_decay)
                set_random_seed(SEED)

                # Create the classifier
                module = policymlp.BuildPolicyMLP(T=5, input_size=784,state_size=128, seed_size=128, query_size=64, number_of_primitives=64, output_size=10)
                module.load_state_dict(torch.load(os.path.dirname(os.path.realpath(__file__)) + model_file))
...
def save_hyperparameters(results, learning_rate, epochs, batch_size, decay_rate):
    results.add_result_lines([
