...
class StepwiseGRU_layer_generator(Paramerized_StepwiseGRU_layer_generator):
    def __init__(self, hidden_size, state_size):
        update_mem = nn.Linear(state_size, state_size)
        update_input = nn.Linear(hidden_size, state_size)
        reset_mem = nn.Linear(state_size, state_size)
        reset_input = nn.Linear(hidden_size, state_size)
        candidate_mem = nn.Linear(state_size, state_size)
        candidate_input = nn.Linear(hidden_size, state_size)
        ...
        super().__init__(hidden_size, state_size, update_mem, update_input, reset_mem, reset_input,
