...
class Paramerized_StepwiseGRU(nn.Module):
    ...
    def __call__(self, x, h):
        number_of_states_to_update = h.size()[1]
...
class Paramerized_StepwiseGRU_layer_generator(nn.Module):
    ...
    # x is batch x obs_size + pos_emb_size, h is batch x state_size x seed_size
...
class StepwiseGRU_layer_generator(Paramerized_StepwiseGRU_layer_generator):
    def __init__(self, state_channels, seed_size):
        update_mem = nn.Linear(seed_size, seed_size)
        update_input = nn.Linear(state_channels, seed_size)
        reset_mem = nn.Linear(seed_size, seed_size)
        reset_input = nn.Linear(state_channels, seed_size)
        candidate_mem = nn.Linear(seed_size, seed_size)
        candidate_input = nn.Linear(state_channels, seed_size)
        ...
        super().__init__(state_channels, seed_size, update_mem, update_input, reset_mem, reset_input,
