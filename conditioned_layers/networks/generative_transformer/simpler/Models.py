...
from conditioned_layers.networks.conditioned_transformer.simpler.Layers import SimplerEncoderLayer
...
class SimplerTransformer(nn.Module):
    ''' Uses the simpler model parts. '''
...
class SimplerEncoder(nn.Module):
    ''' A simpler encoder that uses state update directly as the new state and uses the simpler encoder layer'''
    ...
    def __init__(self, embeddings_vectors, len_max_seq, d_word_vec,
        ...
        self.generation_layer = SimplerEncoderLayer(d_model, d_inner, n_head, d_k, d_v, d_state, dropout=dropout)
    ...
    def forward(self, src_seq, src_pos, return_attns=False):
        ...
        for i in range(self.n_layers):
            enc_output, state, enc_slf_attn = self.generation_layer(
                enc_output, state,
